...
if (/\<script>.*?<\/script>/.test(indexHtml)) {
  const newIndexHtmlString = indexHtml.replace(/\<script>.*?<\/script>/, `<script src="/static/js/${runtimeFileName}"></script>`);

  fs.writeFileSync(indexHtmlPath, newIndexHtmlString);

  console.log(`'${indexHtmlPath}': updated! replace inline script with external. (${runtimeFileName})`);
} else {
  console.log(`'${indexHtmlPath}': have script with external, no need to update`);
}
